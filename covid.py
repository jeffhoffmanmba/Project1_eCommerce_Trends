...
# get_num_of_covid(countries, dur_months, start_year)
...
def get_num_of_covid(countries, dur_months = 5):
    return_df = pd.DataFrame(columns = ["Country", "Date", "Cases"])
    df = pd.DataFrame()
    idx = 0
    ...
    for country in countries:
        ...
        for data in response:
            try:
                ...
                df.loc[idx, "Province"] = data["Province"]
    ...
    group_df = df.groupby(["Country", "Date", "Province"]).max()
    idx = 0
    ...
    for country in df.loc[:, "Country"].unique():
        ...
        return_df.loc[idx, "Cases"] = prev_month_case = group_df.loc[(country, month), "Cases"].sum()
        ...
        for month in df.loc[df.loc[:, "Country"] == country].loc[:, "Date"].unique()[1:]:
            ...
            this_month = group_df.loc[(country, month), "Cases"].sum()
            return_df.loc[idx+1, "Cases"] = this_month - prev_month_case
            prev_month_case = this_month
            idx += 1
        idx += 1
